...
/*
    1. If item weight > sum of weight 
        --> copy weight from same column above row(above cell)
    2. If item weight <= sum of weight
        --> 
            a) Include the new weight and its value + for remaining weight, get value from above row and (sow-w) column
            b) Value by excluding new weight = Value in above cell
            Max(a, b)
*/
...
function knapsack(items, capacity){
    ...
    function getSolution(row,cap){
      ...
      // sow - w < 0
      // The above equation is same as sow < w
      // Which is same as cap < lastItem.w
      if(remaining < 0){
        return lastSolution;
      }
      ...
      // This is the value by excluding new weight
      var lastSolution = row > 0 ? memo[row - 1][col] || NO_SOLUTION : NO_SOLUTION;
      // Refer to the last solution for the remaining capacity,
      // which is in the cell of the previous row with the corresponding(remaining = sow-w) column
      // This is the value 
